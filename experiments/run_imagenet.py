"""
Run ImageNet / ResNet-50 Pruning (v2 - Fixed)
==============================================
Key changes from v1:
  1. No hard TG threshold for block selection — uses greedy target-based selection
  2. Channel pruning as primary compression mechanism for Bottleneck blocks
  3. Joint block+channel optimization matching CIFAR pipeline

ResNet-50 Bottleneck blocks have higher TG than BasicBlocks (min ~0.076 vs ~0.003)
because each block does more work (1x1->3x3->1x1 expansion). This means:
  - Block removal alone cannot achieve high compression without accuracy collapse
  - Channel pruning within surviving blocks is the primary compression lever
  - Strategy: remove a few lowest-TG blocks, then channel-prune the rest

Usage:
    python -m experiments.run_imagenet --data_path /path/to/imagenet \
...
def get_imagenet_loaders(data_path, batch_size=64, num_workers=8):
    """Get ImageNet train/val loaders."""
...
def cosine_similarity_flat(x, y):
    """Compute per-sample cosine similarity between flattened tensors."""
    x_flat = x.flatten(1)
    y_flat = y.flatten(1)
...
def compute_resnet50_block_tg(model, dataloader, device, num_batches=10):
    """
    Compute block-level TG for torchvision ResNet-50.
    Memory-efficient: computes incrementally per block.
    """
    model.eval()
    model.to(device)
    ...
    for layer_name in ['layer1', 'layer2', 'layer3', 'layer4']:
        ...
        for block_idx in range(len(layer)):
            ...
            block_info.append((name, layer[block_idx]))
    ...
    for idx, (name, block) in enumerate(block_info):
        ...
        if name not in tg_scores and total_samples > 0:
            mean_cos = running_cos_sum / total_samples
            tg_scores[name] = 1.0 - mean_cos
...
def compute_resnet50_channel_tg(model, dataloader, device, num_batches=5):
    """
    Compute channel-level importance for each Bottleneck block.
    Measures per-channel activation magnitude at conv2 (3x3 bottleneck conv).
    Returns normalized importance scores per block.
    """
    model.eval()
    model.to(device)
...
class IdentityBottleneck(nn.Module):
    """Identity replacement for Bottleneck blocks."""
    def __init__(self):
        super().__init__()
    def forward(self, x):
        return x
...
def prune_resnet50_blocks(model, blocks_to_remove):
    """Remove Bottleneck blocks from ResNet-50."""
...
def prune_resnet50_channels(model, channel_prune_ratios, channel_tg):
    """
    Prune mid-channels from all 3 convs in Bottleneck blocks.
    Bottleneck: conv1(1x1, Cin->Cmid) -> conv2(3x3, Cmid->Cmid) -> conv3(1x1, Cmid->Cout)
    We prune Cmid consistently across all 3 convs + bn1 + bn2.
    """
    total_pruned = 0
    ...
    for name, ratio in channel_prune_ratios.items():
        ...
        # === Prune conv1 output channels (1x1, Cin -> Cmid) ===
        old_conv1 = block.conv1
        new_conv1 = nn.Conv2d(
            old_conv1.in_channels, num_to_keep,
        ...
        # === Prune conv2 input AND output channels (3x3, Cmid -> Cmid) ===
        ...
        # === Prune conv3 input channels (1x1, Cmid -> Cout) ===
        old_conv3 = block.conv3
        new_conv3 = nn.Conv2d(
            num_to_keep, old_conv3.out_channels,
            kernel_size=1, bias=False
        )
...
def run_imagenet(config):
    """Full TransGap v2 pipeline for ImageNet / ResNet-50."""
    ...
    pruned_state_path = os.path.join(save_dir, f"resnet50_imagenet_f{target_flops}_pruned_state.pth")
    resume_path = os.path.join(save_dir, f"resnet50_imagenet_f{target_flops}_resume.pth")
    summary_path = os.path.join(save_dir, f"resnet50_imagenet_f{target_flops}_summary.json")
    save_path = os.path.join(save_dir, f"resnet50_imagenet_f{target_flops}_best.pth")
    ...
    # Check for resume from pruned state
    if os.path.exists(pruned_state_path):
        ...
        # ========== Stage 1: Block-Level TG ==========
        print("\n" + "=" * 60)
        print("Stage 1: Computing Block-Level TG")
        print("=" * 60)
        ...
        # ========== Stage 2: Channel-Level TG ==========
        print("\n" + "=" * 60)
        print("Stage 2: Computing Channel-Level TG")
        print("=" * 60)
        ...
        # Greedy block selection — NO TG threshold, just pick laziest blocks
        ...
        print(f"  Remaining to cut via channels: {(flops_after_blocks - target_flops_val):,}")
        
        # Channel pruning to reach target — iterative ratio adjustment
        channel_ratios_used = {}
        if flops_after_blocks > target_flops_val:
            # TG-guided base ratios
            ...
            # Start with an estimate and iterate to hit target
            # Channel pruning of ratio r on mid-channels reduces block FLOPs by ~r
            # (conv2 output and conv3 input both shrink)
            flops_gap_ratio = 1.0 - (target_flops_val / flops_after_blocks)
            
            # Iterate: increase ratio until we hit target
            for attempt_ratio in [x * 0.05 for x in range(2, 19)]:  # 0.10 to 0.90
                channel_ratios_used = {}
                ...
                for name in sorted(channel_tg.keys()):
                    ...
                    # Lower TG -> higher pruning (1.3x), higher TG -> lower (0.7x)
                ...
                if test_flops <= target_flops_val * 1.02:  # Within 2% of target
                    break
            ...
            print("  Block removal sufficient — no channel pruning needed")
        
        # ========== Stage 4: Pruned Statistics ==========
        print("\n" + "=" * 60)
        print("Stage 4: Pruned Model Statistics")
        print("=" * 60)
    ...
    # ========== Stage 5: Fine-tuning ==========
    print("\n" + "=" * 60)
    print("Stage 5: Fine-tuning with KD")
    print("=" * 60)
    ...
    start_epoch = 0
    resume_ckpt = save_path.replace('.pth', '_resume.pth') if save_path else resume_path
    ...
    print(f"\n{'=' * 60}")
    print("IMAGENET EXPERIMENT COMPLETE")
    print(f"{'=' * 60}")
    print(f"Baseline:     {baseline_acc:.2f}%")
    print(f"Pre-FT:       {pre_ft_acc:.2f}%")
    ...
    print(f"Time:         {result['total_time']/3600:.2f}h")
...
def main():
    parser = argparse.ArgumentParser(description='TransGap v2 — ImageNet / ResNet-50')
    ...
    setup_logger(args.log_dir, "imagenet_resnet50")
    
    print(f"{'=' * 60}")
    print(f"ImageNet: target_flops={args.target_flops}, max_blocks={args.max_blocks}")
    print(f"{'=' * 60}")
